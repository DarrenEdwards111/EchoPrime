...
\author{Darren J. Edwards, Ph.D.\footnote{Ph.D.\ in Computational Modelling of Cognition.} \\ Founder, Mikoshi Ltd \\ \texttt{mikoshiuk@gmail.com} \\ \\ Mikoshi Ltd -- EchoPrime \\ \texttt{www.echoprime.xyz}}
\date{\today}
...
Safe primes $p$ such that $q = \frac{p - 1}{2}$ is also prime are foundational to many cryptographic protocols, including zero-knowledge proofs \cite{bensasson2014}, multiparty computation \cite{gennaro2003}, secure key exchange \cite{diffie1976}, and digital signature schemes \cite{rivest1978}. Traditional safe prime generation relies on entropy-heavy random sampling followed by probabilistic primality testing. These methods produce correct results but are non-deterministic: given the same inputs, they yield different outputs, and there is no intrinsic record of \emph{why} a particular prime was chosen.
...
The prime counting function satisfies $\pi(x) \sim x / \ln x$, and the density of safe primes follows $\pi_{\text{safe}}(x) \sim C \cdot x / \ln^2 x$, where $C$ is the Hardy--Littlewood twin prime-like constant for safe primes \cite{hardy1923}. The quadratic logarithmic denominator reflects the requirement that both $p$ and $q = (p-1)/2$ must be prime.
...
\]
where $\alpha$ is an empirically calibrated constant that adjusts the projection density, derived from the Bateman--Horn conjecture \cite{bateman1962}. The $\ln^2$ factor compensates for the quadratic thinning of safe primes, and the floor-and-odd construction ensures each candidate is odd. The corresponding Sophie Germain candidate is $\hat{q} = (\hat{p} - 1) / 2$.
...
The scoring mechanism is based on a well-known consequence of Lucas' theorem \cite{lucas1878}: for any prime $p$ and integer $k$ with $1 \leq k \leq T$ (where $T$ is a fixed window size, typically $T = 128$), the binomial coefficient $\binom{p}{k}$ is divisible by $p$. Equivalently, $\binom{p}{k} \equiv 0 \pmod{p}$ for all $1 \leq k \leq p - 1$.
...
After scoring, candidates undergo standard deterministic primality testing \cite{miller1976, rabin1980} as a final confirmation. Both $p$ and $q = (p-1)/2$ must pass primality testing to be published. The collapse score and primality verdict together form the complete verification trace.
...
The hit rate reflects the estimator's ability to target regions of the number line where safe primes cluster. At the 1,000,000-candidate scale, the observed rate was approximately $7.6\%$; independent verification at 10,000 candidates yielded $9.74\%$, consistent with scale-dependent density variation. For context, the asymptotic density of safe primes among odd numbers at comparable scales is approximately $0.23\%$, meaning the estimator is roughly $33$--$42\times$ more efficient than uniform random sampling at the tested scale ($\sim$30-bit candidates).
...
\begin{enumerate}
    ...
    \item Test if $q$ is prime (typically Miller--Rabin \cite{miller1976, rabin1980})
    \item Compute $p = 2q + 1$
...
Zero-knowledge proving systems \cite{bensasson2014} (zkSNARKs, PLONK variants) require trusted setup ceremonies where cryptographic parameters must be generated transparently. EchoPrime allows ceremony coordinators to derive parameters from publicly auditable, collapse-verified primes:
\[
...
Multiparty computation protocols \cite{gennaro2003} require agreement on group parameters, typically safe primes for the group modulus. EchoPrime provides pre-verified, publicly registered safe primes that all parties can independently confirm:
\[
...
Safe primes are directly used in Diffie--Hellman key exchange \cite{diffie1976} (as the group modulus) and in RSA key generation \cite{rivest1978} (as prime factors of $N = pq$). EchoPrime provides a transparent source of such primes with verifiable provenance, suitable for applications where parameter auditability is required.
...
\bibitem{lucas1878}
\'{E}.~Lucas,
``Th\'{e}orie des fonctions num\'{e}riques simplement p\'{e}riodiques,''
\textit{American Journal of Mathematics}, vol.~1, no.~2, pp.~184--196, 1878.
...
\textit{IEEE Transactions on Information Theory}, vol.~22, no.~6, pp.~644--654, 1976.
...
in \textit{Proceedings of the 23rd USENIX Security Symposium}, pp.~781--796, 2014.

\bibitem{gennaro2003}
R.~Gennaro, S.~Jarecki, H.~Krawczyk, and T.~Rabin,
``Secure distributed key generation for discrete-log based cryptosystems,''
\textit{Journal of Cryptology}, vol.~20, no.~1, pp.~51--83, 2007.
(Preliminary version in \textit{Eurocrypt 1999}.)

\bibitem{granville1995}
A.~Granville,
``Harald Cram\'{e}r and the distribution of prime numbers,''
\textit{Scandinavian Actuarial Journal}, vol.~1995, no.~1, pp.~12--28, 1995.
